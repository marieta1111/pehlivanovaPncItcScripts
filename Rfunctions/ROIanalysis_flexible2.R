...
ROIanalysis_flexible2<- function(roiNames,data,medVar){
require(mgcv)

# roiNames:  string vector of names of ROIs for which to do mediation
# data:      dataset
# medVar:    mediating variable to be tested
# covs: covariates in the model

covs<-"sex.o + s(ageAtScan)"
covs1<-"as.factor(race2)+ sex.o + s(ageAtScan)"
...
# Model 2: Mediator = CT + race + sex + age
mdl2<-gam(as.formula(paste0("eval(med) ~ eval(y) +",covs1)), data=data, method="REML")
...
# Model 3: logk = Mediator + CT + race + sex + age 
mdl3<-gam(as.formula(paste0("logk ~ eval(med) + eval(y) +",covs1)), data=data, method="REML")
